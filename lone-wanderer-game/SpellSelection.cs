...
namespace LoneWandererGame
{
    public class SpellSelection
    {
        ...
        private void drawStrings(SpriteBatch spriteBatch, SpriteFont font)
        {
            ...
            spriteBatch.DrawString(font, $"Multiplier: {SpellDefinition.LevelDefinitions[SpellDefinition.CurrentLevel + 1].SpecialMultiplier}", new Vector2(Position.X, Position.Y + texture.Height + 20), Color, 0, Vector2.Zero, 1, SpriteEffects.None, 0.9f);
            spriteBatch.DrawString(font, $"Damage: {SpellDefinition.LevelDefinitions[SpellDefinition.CurrentLevel + 1].Damage}", new Vector2(Position.X, Position.Y + texture.Height + 40), Color, 0, Vector2.Zero, 1, SpriteEffects.None, 0.9f);
        }
    }
}
